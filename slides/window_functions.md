...
Старт контейнера
<pre>
sudo docker-compose --project-name py-db -f docker-compose.yml up --build -d; sudo docker-compose --project-name py-db -f docker-compose.yml run --rm python-db
</pre>

Загрузка данных и старт БД:
<pre>
bash /home/load_data.sh; psql --host $APP_POSTGRES_HOST -U postgres;
</pre>
