...
- Сформируйте запрос (назовём его ЗАПРОС1) к таблице ratings, в котором будут 3 поля
-- movieId
...
Решением домашки будет скрипт ETL.sh вида:

<pre>
#/bin/sh
psql --host $APP_POSTGRES_HOST -U postgres -c "ВАША КОМАНДА СОЗДАНИЯ ТАБЛИЦЫ";

psql --host $APP_POSTGRES_HOST -U postgres -c "ВАША КОМАНДА ЗАЛИВКИ ДАННЫХ В ТАБЛИЦу";

psql --host $APP_POSTGRES_HOST -U postgres -c "ЗАПРОС3";

psql --host $APP_POSTGRES_HOST -U postgres -c "ВАША КОМАНДА ВЫГРУЗКИ ТАБЛИЦЫ В ФАЙЛ"
</pre>
